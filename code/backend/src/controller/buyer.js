...
export const viewBiddingProducts = async (
  ...
  try {
    ...
    const formattedProducts = biddingProducts.map((product) => {
...
//     const bids = await Bidding.find({ buyerId:buyerId }).populate("bidId","name sellerId description basicCost startDate endDate picture");
...
export const viewCompletedBids = async (
  ...
  try {
    ...
    // Find all bids by the buyer and populate bidId with product details
    const bids = await Bidding.find({ buyerId: buyerId }).populate(
      "bidId",
      "name sellerId description basicCost startDate endDate picture"
    );
    ...
    for (let i of bids) {
      // Check if bidId exists and is not null
      if (i.bidId) {
        // Only push the product if it's not already in the array
        if (!myProducts.some((product) => product._id.equals(i.bidId._id))) {
          myProducts.push(i.bidId);
        }
      } else {
        console.log(`Bid with id ${i._id} has a null or missing bidId.`);
      }
    }

    // If myProducts is still empty after processing, send a message
    if (myProducts.length === 0) {
      return res.send({
        msg: "No valid products found for bidding.",
      });
    }
...
export const placeBid = async (req, res) => {
  ...
  try {
    ...
    const getSpecificEmail = await Bidding.findOne({
      bidId: productId,
      buyerId: buyerId,
    })
      .populate("buyerId")
...
export const viewBidHistory = async (
  ...
  try {
    const result = await Bidding.find({ buyerId: buyerid }).populate(
      "bidId",
      "name basicCost endDate"
    ).sort({bidTime:-1});
    const formattedResult = result.map((bid) => {
     
      const bidEndTimeIST = moment(bid.bidId?.endDate).tz("Asia/Kolkata");
    
      const bidEndDate=bidEndTimeIST.format("DD/MM/YYYY");
      const bidEndTime=bidEndTimeIST.format("hh:mm A")
      return {
        ...bid.toObject(),
    endTime:bidEndTime,
        endDate:bidEndDate
...
export const viewMyProductBiddingHistory = async (
  ...
  try {
    const result = await Bidding.find({ bidId: productId, buyerId: buyerId })
      .populate("bidId", "name basicCost endDate")
      .sort({ createdAt: -1 });
